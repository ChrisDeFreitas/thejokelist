...
class JokeListApi {
  ...
  async id( id = null ){
    if( !db.inited() ) 
      throw new Error(`JokeListApi.id() error, database is not initialized.`)

    let sql = `Select ${this.selectCols} from joke `
    if( id === null){   // return first record
      ...
      if( num < this.firstid ) num = this.firstid
      else
      if( num > this.lastid ) num = this.lastid
      sql +=`Where id = ${num} `
    }
    // console.log('sql', sql)
